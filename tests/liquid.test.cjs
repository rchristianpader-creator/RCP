...
function touchPointer(h,type,id,x,y,extra={}){h.fire(type,{pointerType:'touch',pointerId:id,button:0,clientX:x,clientY:y,...extra});}
test('pointer-only touchscreen tap reaches the solver without TouchEvents',()=>{const h=setup();touchPointer(h,'pointerdown',7,100,200);touchPointer(h,'pointerup',7,100,200);h.frame();assert.equal(h.events.filter(e=>e.type==='rcp:liquid-contact'&&e.detail.phase==='start').length,1);assert.equal(h.contacts().length,0);});
test('touch and pointer streams for one tap are not counted twice',()=>{const h=setup();touchPointer(h,'pointerdown',7,100,200);h.touch('touchstart',[[19,100,200]]);touchPointer(h,'pointerup',7,100,200);h.touch('touchend',[]);h.frame();assert.equal(h.events.filter(e=>e.type==='rcp:liquid-contact'&&e.detail.phase==='start').length,1);assert.equal(h.contacts().length,0);});
test('pointer-only fast swipe retains its coalesced and final coordinates',()=>{const h=setup();touchPointer(h,'pointerdown',7,100,600);touchPointer(h,'pointermove',7,200,200,{getCoalescedEvents:()=>[{clientX:130,clientY:450},{clientX:160,clientY:300}]});touchPointer(h,'pointerup',7,250,100);h.frame();const samples=h.events.filter(e=>e.type==='rcp:liquid-contact').map(e=>e.detail);assert.ok(samples.some(p=>p.y===450));assert.equal(samples.at(-1).x,250);assert.equal(samples.at(-1).phase,'end');});
test('new pointer-only tap during momentum uses the new position',()=>{const h=setup();h.touch('touchstart',[[1,20,500]]);h.touch('touchend',[]);h.scroll(200);touchPointer(h,'pointerdown',8,300,200);h.frame();assert.equal(h.contacts().length,1);assert.equal(h.contacts()[0].x,300);assert.equal(h.contacts()[0].y,200);});
test('blur cancels a pending pointer fallback before it can create feedback',()=>{const h=setup();touchPointer(h,'pointerdown',7,100,200);h.fire('blur');h.frame();assert.equal(h.contacts().length,0);assert.equal(h.frames.size,0);});
test('two pointer-only fingers end independently and cancellation releases the last',()=>{const h=setup();touchPointer(h,'pointerdown',7,100,200);touchPointer(h,'pointerdown',8,300,400);h.frame();assert.equal(h.contacts().length,2);touchPointer(h,'pointerup',7,100,200);h.frame();assert.equal(h.contacts().length,1);touchPointer(h,'pointercancel',8,300,400);h.frame();assert.equal(h.contacts().length,0);});
test('late native touch snapshot replaces an already active pointer fallback',()=>{const h=setup();touchPointer(h,'pointerdown',7,100,200);h.frame();assert.equal(h.contacts().length,1);h.touch('touchstart',[[19,110,210]]);assert.equal(h.contacts().length,1);assert.equal(h.contacts()[0].id,'t19');touchPointer(h,'pointercancel',7,110,210);h.frame();assert.equal(h.contacts().length,1);h.touch('touchend',[]);assert.equal(h.contacts().length,0);});
test('trusted click before the pending pointer frame does not double the tap',()=>{const h=setup();touchPointer(h,'pointerdown',7,100,200);touchPointer(h,'pointerup',7,100,200);h.fire('click',{isTrusted:true,detail:1,clientX:100,clientY:200});h.frame();assert.equal(h.events.filter(e=>e.type==='rcp:liquid-contact'&&e.detail.phase==='start').length,1);});
test('water rendering does not hit-test or restyle scrolling cards each frame',()=>{const h=setup();h.document.elementFromPoint=()=>{throw new Error('water animation must not touch the page layout');};h.touch('touchstart',[[1,195,400]]);h.frame();assert.ok(h.pixels().some((v,i)=>i%4===3&&v>0));});
test('viewport height changes preserve waves at their current screen coordinates',()=>{const h=setup();h.touch('touchstart',[[1,195,400]]);h.frame();h.touch('touchend',[]);h.context.innerHeight=760;h.fire('resize');h.frame();assert.ok(h.pixels().some((v,i)=>i%4===3&&v>0));});
