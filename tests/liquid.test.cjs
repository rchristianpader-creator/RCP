...
test('missing canvas context handles touch input without throwing',()=>{const h=setup({canvas:false});h.touch('touchstart',[[1,20,20]]);assert.equal(h.contacts().length,1);assert.equal(h.frames.size,0);});
