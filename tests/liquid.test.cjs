...
test('scroll cleanup observes actual movement before a delayed scroll event arrives',()=>{const h=setup();h.touch('touchstart',[[1,195,500]]);h.scroll(100);h.touch('touchend',[]);h.context.scrollY=500;h.advance(200);h.scroll(650);assert.ok(count(h,'rcp:liquid-scroll')>=3,'movement during the idle timeout must not erase the gesture');});
test('animation frames observe native scroll movement without scroll event delivery',()=>{const h=setup();h.touch('touchstart',[[1,195,500]]);h.touch('touchend',[]);h.context.scrollY=400;h.frame();assert.equal(count(h,'rcp:liquid-scroll'),1);h.advance(1000);const countBefore=count(h,'rcp:liquid-scroll');h.context.scrollY=800;h.frame();assert.equal(count(h,'rcp:liquid-scroll'),countBefore,'settled gesture must not reactivate');});
